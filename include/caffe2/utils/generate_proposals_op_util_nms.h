...
std::vector<int> nms_cpu_upright(
    ...
    float thresh,
    int topN = -1) {
  CHECK_EQ(proposals.rows(), scores.rows());
  CHECK_EQ(proposals.cols(), 4);
  CHECK_EQ(scores.cols(), 1);
  CHECK_LE(sorted_indices.size(), proposals.rows());
...
Eigen::ArrayXXf bbox_overlaps_rotated(
  ...
  CHECK(boxes.cols() == 5 && query_boxes.cols() == 5);
...
std::vector<int> nms_cpu(
    ...
    int topN = -1) {
#if defined(CV_MAJOR_VERSION) && (CV_MAJOR_VERSION >= 3)
  CHECK(proposals.cols() == 4 || proposals.cols() == 5);
  if (proposals.cols() == 4) {
    // Upright boxes
