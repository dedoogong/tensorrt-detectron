...
#include <cfloat>
#include <vector>
//#include "../../../include/bbox_transform_layer.hpp"
...
namespace nvinfer1 {
	...
	Dims BoxTransformLayerPlugin::getOutputDimensions(int index, const Dims * inputs, int nbInputDims)
	{
		...
		return DimsHW(mBoxPredH, mBoxPredW);//top[0]->Reshape(bottom[1]->shape());
	}
	...
	void BoxTransformLayerPlugin::forwardCpu( //const float *const * inputs,
        ...
        Dtype* box_out=(Dtype* )mOutputBuffer;
        ...
        Eigen::Map<const caffe2::ERArrXXf> boxes0((float*)roi_in, mRpnRoisH, mRpnRoisW);//bottom[0]->shape(0), bottom[0]->shape(1));//(1000,4)---------------------------------
        Eigen::Map<const caffe2::ERArrXXf> deltas0((float*)delta_in, mBoxPredH,mBoxPredW);//bottom[1]->shape(0), bottom[1]->shape(1));//(1000,8)
        ...
        Eigen::Map<const caffe2::ERArrXXf> iminfo((float*)iminfo_in, mIminfoH, mIminfoW);//bottom[2]->shape(0), bottom[2]->shape(1));
