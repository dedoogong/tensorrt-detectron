...
const float BBOX_XFORM_CLIP_DEFAULT = 1.79588001734;//log(1000.0 / 16.0);
...
EArrXXt<typename Derived1::Scalar> bbox_transform_rotated(
  ...
  CHECK_EQ(boxes.rows(), deltas.rows());
  CHECK_EQ(boxes.cols(), 5);
  CHECK_EQ(deltas.cols(), 5);
...
template <class Derived1, class Derived2>
EArrXXt<typename Derived1::Scalar> bbox_transform_scale_upright(
    const float scale_before,
    const Eigen::ArrayBase<Derived1>& boxes,
    const Eigen::ArrayBase<Derived2>& deltas,
    const std::vector<typename Derived2::Scalar>& weights =
        std::vector<typename Derived2::Scalar>{1.0, 1.0, 1.0, 1.0},
  ...
  //CHECK_EQ(boxes.rows(), deltas.rows());
  //CHECK_EQ(boxes.cols(), 4);
  //CHECK_EQ(deltas.cols(), 4);

  for(int i=0;i<boxes.rows()/100;i++){
      printf("boxes[%d] : %.2f %.2f %.2f %.2f \n",i, boxes.col(0)[i], boxes.col(1)[i], boxes.col(2)[i], boxes.col(3)[i] );
  }
  EArrX widths = (boxes.col(2) - boxes.col(0))/scale_before + T(1.0);
  EArrX heights = (boxes.col(3) - boxes.col(1))/scale_before + T(1.0);
  auto ctr_x = boxes.col(0)/scale_before + T(0.5) * widths;
  auto ctr_y = boxes.col(1)/scale_before + T(0.5) * heights;

  auto dx = deltas.col(0).template cast<T>() / weights[0];
  auto dy = deltas.col(1).template cast<T>() / weights[1];
  auto dw = (deltas.col(2).template cast<T>() / weights[2]).cwiseMin(bbox_xform_clip);
  auto dh = (deltas.col(3).template cast<T>() / weights[3]).cwiseMin(bbox_xform_clip);
  ...
  T offset(correct_transform_coords ? 1.0 : 0.0); // offset == 1.0
  EArrXX pred_boxes = EArrXX::Zero(deltas.rows(), deltas.cols());
  // x1
  ...
  if(0)
      for(int i=0;i<pred_boxes.rows()/100;i++){
          printf("pred_boxes[%d] : %.2f %.2f %.2f %.2f \n",i, pred_boxes.col(0)[i], pred_boxes.col(1)[i], pred_boxes.col(2)[i], pred_boxes.col(3)[i] );
      }
...
EArrXXt<typename Derived::Scalar> clip_boxes_upright(
  ...
  if(0){
    printf("clip_boxes width, height : %d %d\n", width, height);// 82,82 == int(256/3.125 + 0.5)
    for(int i=0;i<boxes.rows()/100;i++){
        printf("clip_boxes[%d] : %.2f %.2f %.2f %.2f \n",i, boxes.col(0)[i], boxes.col(1)[i], boxes.col(2)[i], boxes.col(3)[i] );
    }

    for(int i=0;i<ret.rows()/100;i++){
        printf("ret_clip_boxes[%d] : %.2f %.2f %.2f %.2f \n",i, ret.col(0)[i], ret.col(1)[i], ret.col(2)[i], ret.col(3)[i] );
    }
  }
  return ret;
}
...
template <class Derived1, class Derived2>
EArrXXt<typename Derived1::Scalar> bbox_transform_scale(
    const float scale_before,
    const Eigen::ArrayBase<Derived1>& boxes,
    const Eigen::ArrayBase<Derived2>& deltas,
    const std::vector<typename Derived2::Scalar>& weights = std::vector<typename Derived2::Scalar>{1.0, 1.0, 1.0, 1.0},
    const float bbox_xform_clip = BBOX_XFORM_CLIP_DEFAULT,
    const bool correct_transform_coords = false,
    const bool angle_bound_on = true,
    const int angle_bound_lo = -90,
    const int angle_bound_hi = 90) {
    CHECK(boxes.cols() == 4 || boxes.cols() == 5);
  if (boxes.cols() == 4) {
    // Upright boxes
    return bbox_transform_scale_upright(
        scale_before, boxes, deltas, weights, bbox_xform_clip, correct_transform_coords);
  } else {
    // Rotated boxes with angle info
    return bbox_transform_rotated(boxes,deltas,weights,bbox_xform_clip,
                                  angle_bound_on,angle_bound_lo,angle_bound_hi);
  }
}
template <class Derived1, class Derived2>
EArrXXt<typename Derived1::Scalar> bbox_transform(
  ...
  if (boxes.cols() == 4) {
    // Upright boxes
    return bbox_transform_upright(boxes, deltas, weights, bbox_xform_clip, correct_transform_coords);
  } else {
    // Rotated boxes with angle info
    return bbox_transform_rotated(boxes,deltas,weights,bbox_xform_clip,
                                  angle_bound_on,angle_bound_lo,angle_bound_hi);
